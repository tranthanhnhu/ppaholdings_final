...
export async function Hero() {
  const t = await getTranslations("whoWeAre.hero");
    ...
    <section className="relative flex h-[640px] flex-col overflow-hidden lg:h-[860px]">
      ...
      <div className="relative z-10 flex h-full flex-col pb-12 lg:justify-between lg:pb-12">
        ...
        <div className="mt-auto flex flex-col gap-3.5 px-8 text-paa-inverse lg:hidden">
          ...
          <p className="pt-2 text-[11px] font-medium leading-[14px] tracking-[1.4px] opacity-50">
            {t("scroll")}
          </p>
        </div>

        <p className="hidden text-center text-[11px] font-medium leading-[14px] tracking-[1.4px] text-paa-inverse opacity-50 lg:block">
          {t("scroll")}
        </p>
      </div>
    </section>
